...
from bot.db.models import User, Code, Payment
...
async def add_code(message: Message, command: CommandObject):
    ...
    if not code_text:
        await message.answer("❌ Iltimos: /addcode KOD123")
        return
    ...
    async with get_session() as session:
        ...
        if existing.scalar_one_or_none():
            await message.answer(f"❌ `{code_text}` kodi mavjud.")
            return
    ...
    await message.answer(f"✅ <code>{code_text}</code> kodi qo'shildi.")
...
async def make_vip(message: Message, command: CommandObject):
    ...
    if not args or not args.isdigit():
        await message.answer("❌ /makevip 123456789")
        return
    ...
    await message.answer(f"✅ <code>{user_id}</code> VIP qilindi.")
...
async def buy_vip_callback(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        ...
        "1️⃣ Click: 123456789\n"
        "2️⃣ Payme: 987654321\n\n"
        "To'lovdan keyin chekni admin ga yuboring."
    )
